...
def model(Y, t, d):
    ...
    for i in range(cols):
        dX_dt[:,i] = - np.dot(L1,XX[:,i]) - np.dot(L2,leader_positions[:,i])
    # print(dX_dt)
...
#         ax.plot(x, y, lw=2, color=colors[k], linestyle=linestyles[count], label=f"agent {k+1}")
#     count+=1
...
for d in [0, dmax]: # 可以修改时滞时间
    ...
    for k in range(num_followers):
        ...
        for i in range(num_iterations):
            x.append(yy[i,:].reshape([num_followers, 2])[k,0])
            y.append(yy[i,:].reshape([num_followers, 2])[k,1]) 
        ax.plot(x, y, lw=2, color=colors[k], linestyle=linestyles[count])
    count+=1
