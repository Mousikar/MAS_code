...
count = 1;
eventtriggeredTimeinterval(1, count) = X(1);
eventtriggeredTimeinterval(2, count) = 0;
...
for i = 2 : stepNum
    ...
    if EventTriggering(e, delt, L) >=0%如果触发函数满足触发条件，就进行系统的更新
        ...
        if count == 1
        eventtriggeredTimeinterval(1, count) = X(i);
        eventtriggeredTimeinterval(2, count) = X(i) - eventtriggeredTimeinterval(1, count);
        else
        eventtriggeredTimeinterval(1, count) = X(i);
        eventtriggeredTimeinterval(2, count) = X(i) - eventtriggeredTimeinterval(1, count-1);
        end
        count = count + 1;
...
figure('name', 'Event-triggered time interval');
stem(eventtriggeredTimeinterval(1, :), eventtriggeredTimeinterval(2, :));%grid on;
xlabel('time/s');
ylabel('||e||');
legend('||e||');
axis([0,timeEnd, 0, 0.9]);
